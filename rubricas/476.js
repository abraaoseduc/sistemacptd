function gerarCalculo476(data, valorPVR, tipoLancamento = 'INCLUSÃO', bloqueado = false, numFolha = '', duasDiferencas = false) {
    ...
    if (duasDiferencas) {
        detalhamentoPlanilha = `${textoLancamento} DE PAGAMENTO DE VALORES SOMADOS DO MÊS CORRENTE E ANTERIOR DE DIFERENÇAS REFERENTE A PVR ${stringCalculosTexto}, DEVIDO RESCISÃO DE CONTRATO EM ${data.dtalt}${complementoBloqueio}`;
    } else {
        detalhamentoPlanilha = `${textoLancamento} DE PAGAMENTO DE DIFERENÇAS REFERENTE A PVR ${stringCalculosTexto}, DEVIDO RESCISÃO DE CONTRATO EM ${data.dtalt}${complementoBloqueio}`;
    }
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        botoesFolhaHTML += `
            <button type="button" 
                    onclick="selecionarFolha476('${data.cardId}', '${i}')"
            ...
            <div class="folha-selector" style="margin: 0.75rem 0; font-size: 0.85rem; background: #ffffff; padding: 0.5rem; border: 1px solid #cbd5e1; border-radius: 4px;">
                ...
                <div>
                    <label style="cursor: pointer; font-weight: 500; display: inline-flex; align-items: center; gap: 0.3rem;">
                        <input type="checkbox" id="chk_bloqueio_476_${data.cardId}" ${bloqueado ? 'checked' : ''} onchange="atualizarBloqueioFolha476('${data.cardId}')"> BLOQUEIO DE PAGAMENTO REALIZADO NA FOLHA
                    </label>
                </div>

                <input type="hidden" id="num_folha_476_${data.cardId}" value="${numFolha}">

                <div id="painel_folhas_476_${data.cardId}" style="display: ${bloqueado ? 'grid' : 'none'}; grid-template-columns: repeat(6, 1fr); gap: 0.35rem; margin-top: 0.5rem;">
                    ${botoesFolhaHTML}
                </div>
            </div>
...
function atualizarBloqueioFolha476(cardId) {
    const chk = document.getElementById(`chk_bloqueio_476_${cardId}`);
    const painel = document.getElementById(`painel_folhas_476_${cardId}`);
    const inputHidden = document.getElementById(`num_folha_476_${cardId}`);
    ...
    if (typeof atualizarBloqueioFolha === 'function') {
        atualizarBloqueioFolha(cardId, '476');
    }
}

function selecionarFolha476(cardId, numFolha) {
    const inputHidden = document.getElementById(`num_folha_476_${cardId}`);
